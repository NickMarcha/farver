...
public static class CommonExtensions
{
    ...
    /// <param name="mb"></param>
    /// <returns></returns>
    public static bool OnGround2D(this UnityEngine.MonoBehaviour mb)
    {
        return OnGround2D(mb.gameObject, new string[0]);
    }

    public static bool OnGround2D(this UnityEngine.MonoBehaviour mb, params string[] layers)
    {
        return OnGround2D(mb.gameObject, layers);
    }

    public static bool OnGround2D(this GameObject obj)
    {
    ...
    /// <returns></returns>
    public static bool OnGround2D(this GameObject obj, params string[] layers)
    {
        Collider2D collider = obj.GetComponent<Collider2D>();

        //If the game object does not have a collider we can asume it isn't grounded as it won't interact with it
        if (collider == null)
        {
            return false;
        }
        ...
        if (layers.Any())
        {
            return Physics2D.Raycast(new Vector2(collider.bounds.min.x, collider.bounds.min.y - 0.1f), Vector2.right, collider.bounds.size.x, LayerMask.GetMask(layers));
        }

        bool temp = Physics2D.Raycast(new Vector2(collider.bounds.min.x, collider.bounds.min.y - 0.1f), Vector2.right, collider.bounds.size.x);
        if (obj.name == "Fireball" && temp)
        {
            Debug.Log("Fireball is on ground yo");
        }
        return temp;
